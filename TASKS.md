...
### TASK-546: [HARD][IMPL] Wave 5 performance characterization suite (`bench-perf`)
**Output**: `benches/perf/`, `docs/design/perf-suite.md`, `docs/perf/wave5-results.md`
**Depends on**: TASK-526 (Wave 5 benches), TASK-536 (per-shard dispatch), TASK-537 (worker timing)
**Description**: Build a compelling performance suite that demonstrates bqlite at 100M / 1B / 10B-row scales, suitable for headline-throughput claims. Today's benches (`benches/wave[2-5]/*`) are correctness/regression tripwires sized for CI runs in seconds; they do not produce the absolute rows-per-second / GB-per-second numbers a perf characterization needs and do not scale beyond the `BenchMode::Reference` 100M cap baked into `BenchSizing`. This task extends the existing harness rather than replacing it. Required: (a) **Scale ladder.** Add a `BenchScale::{Small, Medium, Large, XLarge}` enum to `benches/common/mod.rs` covering 1M / 100M / 1B / 10B rows, selectable via `BQLITE_BENCH_SCALE` env var (default `Small`). Keep `BenchMode::Reference` semantics intact — `Reference` mode plus the chosen `BenchScale` controls hard-target enforcement vs sample-and-report. (b) **Streaming, deterministic event generator.** A new `StreamingEventGenerator` produces events in fixed-size chunks (default 1M events / chunk) so generation never holds more than one chunk in RAM. Seeded power-law entity skew (a small percentage of entities own the majority of events), 20 event types, the existing 7-property column profile. Output goes directly into the partitioner / writer pipeline rather than `Vec<Event>`. (c) **Persistent fixture cache.** A `PersistentFixture` helper materializes a fully-ingested `Database` to disk under `$BQLITE_BENCH_CACHE_DIR` (default `target/bench-fixtures/`) keyed by `(scale, seed, schema_version)`. First run builds; subsequent runs reuse. A `--regen` env flag forces rebuild. Stale fixtures (older than 90 days or with mismatched schema version) are rebuilt automatically. (d) **New bench group: `perf_scan_selectivity`** — filter-then-aggregate throughput across selectivity levels 0.001 / 0.01 / 0.1 / 0.5 / 1.0, reports rows/s and GB/s. (e) **New bench group: `perf_aggregation_cardinality`** — `COUNT(*) GROUP BY` across group-cardinality 10 / 1K / 1M / 100M (the last one only at Large+). Reports rows/s and groups-emitted/s. (f) **New bench group: `perf_funnel_depth`** — 2-step / 5-step / 10-step funnels with `WITHIN 7d`, both relative and absolute time ranges. Reports entities-processed/s and events-processed/s. (g) **New bench group: `perf_sort_topk`** — full sort vs. sort+limit(1000) vs. sort+limit(1M), reports time-to-first-row and total time. Exercises spill at Medium+ scales. (h) **New bench group: `perf_ingest_throughput`** — INSERT VALUES, JSONL, and Parquet at all scales, reports rows/s and bytes-ingested/s. (i) **New bench group: `perf_mc_scaling`** — same query at `query_threads=1/2/4/8/auto` to produce the speedup curve; runs the `perf_scan_selectivity` query at selectivity 0.1. (j) **New bench group: `perf_concurrent_queries`** — 1 / 4 / 16 parallel queries against the same Database, reports aggregate throughput and per-query p50/p99 latency. (k) **Report generator.** A small binary in `benches/src/bin/perf_report.rs` reads `target/bench-results.json` and produces `docs/perf/wave5-results.md` — a single document with: headline-numbers summary table at the top, per-bench-group section with absolute rows/s and GB/s tables across all 3 scales, multi-core scaling chart (ASCII or Markdown table), notes on memory budget / spill behavior per scale, and a `## Methodology` section citing fixture seed, hardware (Apple M2 Max for the headline run), `BQLITE_BENCH_SCALE`, and Criterion config. Final XLarge (10B) run is a one-shot manual invocation; design must tolerate it taking hours. Out of scope: comparison vs. external engines (DuckDB / ClickHouse / Polars) — explicitly deferred to a future task once raw numbers are established. Out of scope: pretty HTML rendering — Markdown only.
