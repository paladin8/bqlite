...
### 5.1 `finish_entity_into` — Overrides And Default Path

For each fusion-eligible stateful operator, this document records whether the operator carries an explicit `finish_entity_into` override or relies on the default-trait path (which calls `finish_entity` to materialize a per-entity `RecordBatch` and feeds it to `Accumulator::update_batch`). After TASK-520 every Wave 4 operator participates in fusion; the only choice is between an explicit override (MATCH only, today) and the default path (the three Wave 4 stateful operators).

| Operator           | Path                  | Rationale | Source             |
| ------------------ | --------------------- | --------- | ------------------ |
| `SequenceMatch` (MATCH) | **Explicit override** — TASK-321 | The fused match-aggregate is the canonical example: the per-row hot loop has zero demand-related branches and the layered-extraction hooks (§4.2 of execution-model.md, §7.5 of planner-pipeline.md) reach the accumulator without rebuilding an intermediate `RecordBatch`. The shape lives at `crates/bqlite-operators/src/matcher/mod.rs:377`. | match-operator.md §3, sequence-matching.md §13.4 |
| `Sessionize`       | **Default path** — TASK-520    | The default `finish_entity_into` calls `finish_entity` to produce a native per-session `RecordBatch` and feeds it to `update_batch`. The session-fold candidates in operators/sessionize.md §10.1 (session-fold count, sum/avg over forwarded columns) are perf-only and remain follow-on work; correctness wise the default path matches the unfused result. | operators/sessionize.md §10 |
| `EventSelect` (FIRST/LAST/NTH) | **Default path** — TASK-520 | Per-entity state is a single candidate row, so the intermediate `RecordBatch` is one row × *N* columns. The default path is correct; the §12.1 benchmark-gated optimization (skip the 1-row batch and call `Accumulator::update(group_key, &values)` directly) is a follow-on. | operators/event-select-sample.md §12 |
| `Attribute`        | **Default path** — TASK-520    | ATTRIBUTE already emits flat per-touchpoint rows, so the default path is the right shape. The §7.4.4 row-shape table in planner-pipeline.md is now the runtime contract. The `WHERE touchpoint_ts IS NOT NULL`-then-aggregate row in that table remains deferred (FilterThenAggregate is out of scope). | operators/attribute.md §13 |

Two decisions are baked into this matrix and are worth surfacing:

- **MATCH is the only stateful operator that uses an *explicit* override.** This was a Wave 3 decision — sequence-matching.md §13.4 specified it and the operator implementation has been live since TASK-321. The other three operators ride the default path; the v1 perf delta is acceptable because the per-entity output cardinality is bounded (one row per session for SESSIONIZE, one row per entity for EventSelect, one row per qualifying touchpoint for ATTRIBUTE).
- **All four operators participate in fusion after TASK-520.** The planner-side detection in `crates/bqlite-planner/src/opt/fuse_match_aggregate.rs` now handles `Aggregate(SequenceMatch | Sessionize | EventSelect | Attribute)` adjacency and emits a fused descriptor. The pre-fusion native schema is preserved on the descriptor in a new `pre_fusion_output_schema` field so the runtime operator keeps building per-entity batches in the native shape.
...
| TASK-520 | **Landed.** Activate stateful-to-aggregate fusion for SESSIONIZE / EventSelect / ATTRIBUTE per planner-pipeline.md §7.4.2 / §7.4.3 / §7.4.4. The three Wave 4 operators ride the default `finish_entity_into` path (per §5.1 above) but now: (a) accept a populated `fused_aggregate` instead of asserting it `None`; (b) preserve the operator's native output schema in a new `pre_fusion_output_schema` field so the runtime keeps building per-entity batches in the native shape; (c) advertise `supports_aggregation_fusion: true`. The shared `EntityOperatorAdapter` gained a fused branch (mirroring `SequenceMatchAdapter::FusedAccState`). The planner pass `fuse_match_aggregate` was extended to detect `Aggregate(Sessionize | EventSelect | Attribute)` adjacency, with a B4 bugfix to `is_simple_column_ref` that now also gates group-by expressions (preventing a fused-runtime panic that the existing MATCH path was incidentally avoiding). |
