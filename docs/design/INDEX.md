...
- **operators/event-select-sample.md** — `EventSelectOperator` (FIRST/LAST/NTH) + SAMPLE architecture: `EventSelectKind` parameterization, per-entity candidate row state, event-type list extension, WHERE per-event filtering order, same-`ts` tie-breaking by `__seq_id`, `lookback:` scan-range widening, omitted-entity rules, FIRST/NTH early termination, SAMPLE `fraction:`-only surface (no `count:`), xxHash64 entity-id hashing with stability contract, population-invariance under upstream stateless filters, scan-pushdown contract, demand-driven column forwarding, composition rules with SESSIONIZE and chained selects, Wave 5 fusion deferral (TASK-411, Wave 4)
...
- **planner/optimizer-direction.md** — Wave 5 optimizer-policy reconciliation: rule-based architecture preserved, narrow heuristic gating admitted, five-source `PlannerStats` snapshot (catalog aggregates, index registry booleans, cohort sizes), explicit non-sources (NDV sketches, histograms, selectivity functions), per-rule policy matrix (Passes 1–10), plan-time vs post-cohort phase split, EXPLAIN visibility contract, reconciliation against planner-pipeline.md §1.2/§6.1/§6.2/§14 (TASK-504, Wave 5)
...
- **storage/zero-copy-scan-filter.md** — encoded-preserving read-path design: copy budget, borrowed `EncodedColumnView` / stitched-row surfaces, zero-copy dictionary and RLE filtering, late materialization boundary for scan/filter (future follow-up design, Wave 5)
...
- **engine/memory-budget.md** — Query-time memory enforcement model: default budget reconciliation (3 GiB query / 800 MiB compaction / 256 MiB ingest, ~4 GiB aggregate), tracked vs. untracked allocation classes, `MemoryBudget` ↔ `QueryContext` wiring, per-operator spill-vs-fail policy, RAII reservation lifecycle, configuration surface (TASK-501, Wave 5)
- **engine/operator-fusion.md** — Fused stateless segment + operator-fusion contract: `StatelessKernel` trait, `materialize_filtered_batch` boundary helper, `FusedStatelessSegment` driver, sparsity / segment-boundary / aggregate-handoff materialization triggers, stateless-vs-stateful fusion boundary, v1 `finish_entity_into` decisions for MATCH / SESSIONIZE / EventSelect / ATTRIBUTE, follow-on TASK-518/519/520 scope (TASK-503, Wave 5)
...
- **engine/morsel-scheduler.md** — Morsel scheduler & query/compaction capacity sharing: morsel generation algorithm with adaptive halving, lock-free per-query MPMC queue, FIFO query queuing on `CoreBudget`, per-shard `AccumulatorHandle` Mutex with per-entity grain, query-side `CoreBudget` permit acquisition, Wave 5 skew/parallelism metrics + sampling protocol, panic/cancellation/timeout interaction at the morsel boundary, follow-on TASK-523/524/525/526 scope (TASK-506, Wave 5)
- **engine/spill.md** — Spill-to-disk protocol: v1 spill surface (sort + ingest partitioner spill; cohort/IN-subquery and aggregate fail fast), spill-root directory and configuration, per-purpose Arrow-IPC file layouts, `<spill_root>/<query_id>/<purpose>-<seq>.spill` path scheme, RAII cleanup vs. cancellation/timeout/panic exit paths, engine-open `rm_rf` crash-recovery model, operator participation protocol via `register_spill_handler`, reconciliation with execution-model.md §10.3 and cancellation.md §5.2/§5.3 (TASK-502, Wave 5)
