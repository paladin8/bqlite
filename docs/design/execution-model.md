...
**Wave 5 implementation status (TASK-524 / TASK-537).** The per-query opt-in flag (`QueryOptions::collect_cpu_metrics`), the per-worker perf-counter seam (`bqlite_engine::PerfCounters::open_or_disabled`), and the `QueryMetrics::cpu_metrics_enabled` round-trip all ship in Wave 5. TASK-537 lands the Linux `perf_event_open` integration: when the engine has opted into CPU-cost metrics and the kernel grants the syscall (i.e. `CAP_PERFMON` or `perf_event_paranoid` low enough), each worker opens a perf-event group covering `PERF_COUNT_HW_CPU_CYCLES`, `PERF_COUNT_HW_BRANCH_MISSES`, and `PERF_COUNT_HW_CACHE_LL/READ/MISS`, reads it once per morsel boundary, and folds the deltas into `branch_misses` / `llc_misses` / `total_cpu_cycles`. On a Linux box where the syscall is refused or on macOS/other platforms (the v2 `kpc` integration remains follow-on), `PerfCounters::open_or_disabled` returns the disabled variant; the rendered CLI surface labels the rows as `not collected (no CAP_PERFMON)` so the absence is unambiguous.

Worker timing rows (`worker_busy_ns_*`, `worker_idle_ns_*`, `entity_event_skew_p99`) carry real values once TASK-537 lands. v1 `entity_event_skew_p99` reports the per-worker p99-vs-p50 spread of per-morsel processed-event counts as a proxy for the per-entity sample documented in `morsel-scheduler.md` §8.2; the per-entity sample lands when the `EntityOperatorAdapter` `finish_entity` hook is wired. See `morsel-scheduler.md` §8.2 for the v1 simplifications and the migration path.
