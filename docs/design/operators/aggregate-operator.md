...
**Bounded memory with no spill (v1 constraint).** Group cardinality is hard-capped at `max_groups` (default 1,000,000). At ~100 bytes per group, 1M groups fit within the 3 GB query budget. Spill-to-disk is deferred to v2. The cap is enforced inside `HashAccumulator::update` and surfaces as a typed error.
...
There is no spill-to-disk for aggregation state in v1. At ~100 bytes per group, 1M groups occupy ~100 MB — well within the 3 GB query budget.
