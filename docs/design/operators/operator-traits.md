...
| `EntityOperator::supported_demands` + `DemandCapabilities` (§4, §8) | **Deferred to TASK-110.** | TASK-110 introduces the `DemandCapabilities` scaffold as a placeholder enum + propagation trait; it depends on this task so it can extend the trait additively. |
...
1. **`DemandCapabilities` / `supported_demands()`** — TASK-110 extends `EntityOperator` with a defaulted `supported_demands` method returning a placeholder enum. The real demand propagation protocol is a Wave 4+ design task.
